...
CONTROL = [False, False, False, True, False]  # 数据集控制位，True表示计算该数据集，False反之 （用于多服务器训练）
NUMCLASSES = 2
DEVICE = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
print('DEVICE: ', DEVICE)

ModelName =  ["MTCN"]
# ModelName =  ["EEGInception"]
# ModelName =  ["VisionEagle"]
# ModelName = ["MEDGNN"]
ModelType = [True]
Model = [MTCN(NUMCLASSES)]
# Model = [EEGInception(NUMCLASSES)]
# Model = [VisionEagle(NUMCLASSES)]
# Model = [MEDGNN(NUMCLASSES)]
...
if __name__ == '__main__':
    ...
    for dataset_num in range(len(sub_num)):
        ...
        # for subject_id in range(1, 2):  # 只选择一个数据进行测试代码，最后放开全部的数据进行训练
            # 数据集中的被试层
            # 完成数据加载操作
            for idx in range(len(ModelName)):
                ...
                # 结果以及模型保存-非MTCN模型的话，得写结果的excel文件才行，要不然会报错
