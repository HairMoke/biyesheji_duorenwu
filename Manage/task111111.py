        ...
        with tqdm(total=epochs, desc='Epoch', leave=True, unit_scale=True) as pbar:
            for epoch in range(epochs):
                ...
                for index, data in enumerate(dataloader):
                    ...
                    pred_ftr, loss_ftr = self.model(x_train_ftr_task, "ftr")  # (160,10)
    ...
    def goTask(self):
        ...
        if self.model_name == 'VisionEagle':
            if (self.task_type == True):
                ...
                general.test(self.data)
        if self.model_name == 'MEDGNN':
            if (self.task_type == True):
                general = MEDGNNTrainTask(self.sub_id, self.model, self.device, self.dataset_name)
                general.train(self.data, self.epoch)
            else:
                general = MEDGNNTestTask(self.sub_id, self.model, self.device, self.dataset_name) # 测试走通用模型即可
                general.test(self.data)
        ...
        with tqdm(total=epochs, desc='Epoch', leave=True, ncols=100, unit_scale=True) as pbar:
            for epoch in range(epochs):
                ...
                for index, data in enumerate(dataloader):
                    ...
                    x = torch.tensor(x).to(torch.float32)
                    y = torch.tensor(y).to(torch.long)
                    x, y, padding_mask = x.to(self.device), y.to(self.device), padding_mask.to(self.device)
                    y_pred = self.model(x, padding_mask, None, None)  # batch_x, padding_mask, None, None
...
class MEDGNNTestTask:
    ...
    def test(self, data):
        ...
        for index, data in enumerate(data):
            ...
            y = torch.tensor(y).to(torch.long)
            x, y, padding_mask = x.to(self.device), y.to(self.device), padding_mask.to(self.device)
            y_pred = self.model(x, padding_mask, None, None)
            _, pred = torch.max(y_pred, 1)
