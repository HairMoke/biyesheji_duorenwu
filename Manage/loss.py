...
class LossFunction(nn.Module):
    def __init__(self):
        ...
        self.loss = AutomaticWeightedLoss(4)
    
    
    def calculateTrainStageLoss(self, pred_primary, label_primary, pred_vto, label_vto, pred_msp, label_msp, pred_ftr, label_ftr):
        ...
        loss_ftr = self.criterion(pred_ftr, label_ftr.type(torch.cuda.LongTensor))

        loss = self.loss(loss_primary, loss_vto, loss_msp, loss_ftr)

        return loss

    def calculateTestStageILoss(self, pred_vto, label_vto, pred_msp, label_msp,pred_ftr, label_ftr):
        loss_vto = self.criterion(pred_vto, label_vto.type(torch.cuda.LongTensor))
        loss_msp = self.criterion(pred_msp, label_msp.type(torch.cuda.LongTensor))
        loss_ftr = self.criterion(pred_ftr, label_ftr.type(torch.cuda.LongTensor))
        
        loss = self.loss(loss_vto, loss_msp, loss_ftr)
