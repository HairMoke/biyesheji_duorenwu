...
def collate_fn(data, max_len=None):
    ...
    lengths = [
        x_total.shape[0] for x_total in features
    ...
    for i in range(batch_size):
        ...
        # x_total[i, :end, :] = features[i][:end, :]
        x_total[i, :end, :] = torch.from_numpy(features[i][:end, :]).float()  # 关键修改：转换为 PyTorch 张量

    # targets = torch.stack(labels, dim=0)  # (batch_size, num_labels)
    targets = torch.stack([torch.from_numpy(y).float() for y in labels], dim=0)  # 确保 labels 也是张量

    # padding_masks = padding_mask(torch.tensor(lengths, dtype=torch.int16), max_len=max_len)  # (batch_size, padded_length) boolean tensor, "1" means keep
    padding_masks = padding_mask(torch.tensor(lengths, dtype=torch.int16), max_len=max_len)

    return x_total, targets, padding_masks
