...
from module_test.gtn.encoder import Encoder
import math
...
if __name__ == '__main__':
    ...
    input = torch.randn(32,256,64)  # 生成随机输入
    model = Transformer(d_model=512, d_input=256, d_channel=64, d_output=2, d_hidden=1024,
                  q=8, v=8, h=8, N=8, dropout=0.2, pe=True, mask=True, device=device)  # 实例化TripletAttention
    output = model(input, 'train')
    print(output.shape)
