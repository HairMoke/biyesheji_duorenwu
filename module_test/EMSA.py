...
if __name__ == '__main__':
    ...
    input = torch.rand(256, 64, 256).cuda()  # 随机生成输入数据
    # 使用插值方法调整尺寸
    x_resized = F.interpolate(input.unsqueeze(0), size=(64, 64, 512), mode='trilinear', align_corners=True).squeeze(0)
    print(x_resized.shape)  # 输出应为 torch.Size([64, 64, 512])
